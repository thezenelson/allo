...
class BPETokenizer:
    ...
    def __init__(self):
        ...
        try:
            self.decoder_list = np.load("gpt-2-124M-keys.npy")
        except:
            print("building decode table")
            self.decoder_list = [*self.encoder.keys()]
            for i in range(0, 256):
                self.decoder_list = np.char.replace(self.decoder_list, self.byte_encoder[i], chr(i))
            np.save("gpt-2-124M-keys", self.decoder_list)
    ...
    def decode_list(self, tokens):
        return_str = ""
        for token in tokens:
            return_str += self.allo_decode(token)
        return return_str

    def allo_decode(self, token):
        return self.decoder_list[token]
...
class GPT2(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, temperature: float = 0.1, top_k: int = 40):
        ...
        # print(f"max: {torch.sort(logits)}")

        # probs = torch.nn.functional.softmax(logits, dim=-1)
        probs = soft_max.soft_max(logits)
        return torch.multinomial(probs, num_samples=1)

prompt = 'The Eiffel tower is in' if len(sys.argv) <= 1 else sys.argv[1]
model = GPT2('gpt2-medium').eval()
...
print(encoding.decode_list(context), end='', flush=True)
context = torch.tensor([context], dtype=torch.long)
for _ in range(50): # max_tokens
    y = model(context)
    context = torch.cat((context, y), dim=1)
    w = encoding.allo_decode(y[0,0])
